...
export class HoshimiLyricsManager extends LyricsManager {
	public override async current(
		guildId: string,
		skipSource?: boolean,
	): Promise<LyricsResult | null> {
		if (this.node.isNodelink()) {
			const player: PlayerStructure | undefined =
				this.node.nodeManager.manager.getPlayer(guildId);
	...
	public override async get(
		...
		if (this.node.isNodelink()) {
			const lyrics: NodelinkLyrics | null =
				await this.node.rest.request<NodelinkLyrics>({
