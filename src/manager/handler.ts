...
const customImport = <T>(path: string): Promise<T> =>
	import(`${pathToFileURL(path)}?update=${Date.now()}`).then(
		(x) => x.default ?? x,
	) as Promise<T>;
...
export class LavalinkHandler extends BaseHandler {
	 ...
	 */
	readonly values: Map<LavalinkEventNames, Lavalink> = new Map<
		LavalinkEventNames,
		Lavalink
	>();
	...
	public async start(): Promise<void> {
		const files = await this.loadFilesK<{ default: Lavalink }>(
			await this.getFiles(
				await this.client.getRC().then((x) => x.locations.lavalink),
			),
		);
		...
		for (const file of files) {
			...
			const run = (...args: LavalinkEventParameters) =>
